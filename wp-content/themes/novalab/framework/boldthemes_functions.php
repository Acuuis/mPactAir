...
if ( ! function_exists( 'boldthemes_enqueue_custom_fonts' ) ) {
	function boldthemes_enqueue_custom_fonts() {
		foreach ( BoldThemesFramework::$custom_fonts_enqueue as $item ) {
			...
			foreach ( $item['variants'] as $variant_key => $variant_value ) {
				echo '<link rel="preload" href="' . esc_url( $variant_value ) . '" as="font" crossorigin>';
				echo '<style>';
				echo '@font-face{';
					echo 'font-family:"' . esc_html( $font_family ) . '";';
					if ( strpos ( $variant_key, 'italic' ) !== false ) {
					...
					echo 'src:url(' . esc_url( $variant_value ) . ')format("truetype");';
				echo '}';
				echo '</style>';
			}
		}
	...
	function boldthemes_get_custom_fonts( $arr ) {
		...
		if ( $glob_match ) {
			foreach( $glob_match as $file ) {
				if ( is_dir( $file ) ) {
					$glob_match_file = glob( $file . '/*.[tT][tT][fF]' );
					if ( $glob_match_file ) {
						preg_match( '/[^\\\\\/]+$/', $file, $match );
						preg_match( '/[^\\\\\/]+\.ttf$/i', $glob_match_file[0], $match_file );
						BoldThemesFramework::$custom_fonts[ $match[0] ] = array(
							'font' => $match[0],
							'variants' => array(
								'regular' => $base_uri . '/custom-fonts/' . $match[0] . '/' . $match_file[0]
							)
						);
					}
				}
...
boldthemes_get_custom_fonts( array( 'base_dir' => get_template_directory(), 'base_uri' => get_template_directory_uri() ) );
boldthemes_get_custom_fonts( array( 'base_dir' => get_stylesheet_directory(), 'base_uri' => get_stylesheet_directory_uri() ) );
...
if ( ! function_exists( 'boldthemes_header_headline' ) ) {
	function boldthemes_header_headline( $arg = array() ) {
		...
		if ( $title != '' || $supertitle != '' || $subtitle != '' ) {
			$extra_class .= $feat_image ? ' bt_bb_background_image ' . apply_filters( 'boldthemes_header_headline_gradient', 'bt_bb_background_overlay_dark_solid' ) . ' ' . $parallax_class . ' ' . apply_filters( 'boldthemes_header_headline_skin', 'bt-dark-skin' ) . ' ' : ' ';
